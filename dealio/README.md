...
**Dealio is a fictional CRM for sales teams — with a real, live agent inside.**
It is one of guuey's public demos: a complete agentic app — product frontend,
brand config, and a hosted agent — where the entire agent is a declarative
...
- **No `mcpServers` key.** The ggui generative-UI rail is the _platform
  default_ and merges in automatically — that is how this agent renders stage
  boards, follow-up drafts, and deal summaries without declaring a single MCP
...
| `web`        | :6890 | the Dealio product frontend (Vite + React on `@guuey/chat`): landing · login · pipeline pages · the chat |
...
npx guuey apps create --name my-dealio    # create an app (creating is free)
...
This directory is the source of truth for the hosted Dealio demo agent — the
same definition, deployed per environment:

| Environment          | App id                                 |
| -------------------- | -------------------------------------- |
| production (release) | `ba9ab3df-c663-47cc-87dc-d9591a872321` |
| dev (sandbox)        | `5a2e2eeb-bf3d-4845-8b6f-2408078b8941` |
...
demo chrome — the honesty strip telling visitors that Dealio is a fictional
