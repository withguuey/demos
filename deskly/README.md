...
**Deskly is a fictional helpdesk SaaS — with a real, live agent inside.** It is
...
- **No `mcpServers` key.** The ggui generative-UI rail is the _platform
  default_ and merges in automatically — that is how this agent renders ticket
  queues, SLA summaries, and drafted replies without declaring a single MCP
...
| `web`        | :6890 | the Deskly product frontend (Vite + React on `@guuey/chat`): landing · login · helpdesk pages · the chat |
...
npx guuey apps create --name my-deskly    # create an app (creating is free)
...
This directory is the source of truth for the hosted Deskly demo agent — the
same definition, deployed per environment:

| Environment          | App id                                 |
| -------------------- | -------------------------------------- |
| production (release) | `fcac88b7-9e95-45aa-968d-7e703fdc7bbc` |
| dev (sandbox)        | `9c70f696-de5a-45c3-836e-b4e0b664856d` |
...
demo chrome — the honesty strip telling visitors that Deskly is a fictional
