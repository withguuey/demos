 ...
 *
 * Look: Precision Grid (founder-picked direction C) — carbon ground,
 * hairline rules, amber signal; the board panel is drawn straight from the
 * same fixtures the agent talks about.
 */
...
import { STATS } from "../lib/deskly-fixtures";
    ...
    <main className="page landing">
      <section className="hero" data-tour="hero">
        <div className="hero-copy">
          <p className="micro">Support operations · triage, by conversation</p>
          ...
          <div className="hero-asks" aria-label="Example things to ask">
            <span className="ask-chip">&ldquo;What&rsquo;s about to breach?&rdquo;</span>
            <span className="ask-chip">&ldquo;Draft a reply to #4228&rdquo;</span>
            <span className="ask-chip">&ldquo;Who&rsquo;s carrying the heaviest load this week?&rdquo;</span>
          </div>
        ...
        <aside className="hero-board" id="board" aria-label="Today's numbers">
          <p className="micro">The board</p>
          <div className="board-stats">
            {STATS.map((stat) => (
              <div key={stat.label} className="board-stat">
                <p className="board-stat-label">{stat.label}</p>
                <p className="board-stat-value">{stat.value}</p>
              </div>
            ))}
          </div>
          <p className="board-quote">&ldquo;show me the two tickets about to breach&rdquo;</p>
        </aside>
      </section>
      {appConfig.demoMode ? (
        <p className="demo-strip">
          Deskly is a fictional product — a live demo of an agentic app built on guuey. Every
          ticket, name, and number on this site is mock data.
        </p>
      ) : null}
    </main>
  );
