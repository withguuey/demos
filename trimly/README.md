...
**Trimly is a fictional booking SaaS — with a real, live agent inside.** It is
...
- **No `mcpServers` key.** The ggui generative-UI rail is the _platform
  default_ and merges in automatically — that is how this agent renders week
  grids, slot comparisons, and plan summaries without declaring a single MCP
...
| `web`        | :6890 | the Trimly product frontend (Vite + React on `@guuey/chat`): landing · login · booking pages · the chat  |
...
npx guuey apps create --name my-trimly    # create an app (creating is free)
...
This directory is the source of truth for the hosted Trimly demo agent — the
same definition, deployed per environment:

| Environment          | App id                                 |
| -------------------- | -------------------------------------- |
| production (release) | `53e3b162-fb0e-4ad1-88ec-ff3bcc505336` |
...
demo chrome — the honesty strip telling visitors that Trimly is a fictional
